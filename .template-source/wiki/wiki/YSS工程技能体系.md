...
词汇上的分层是：核心技能默认可发现，负责生命周期控制或通用研发入口；专项技能由 Router 按影响面和实现合同按需选择；试验技能只在明确试验范围内使用，不进入 Router 默认技能闭包。`.template-spec/agents/yss-skill-registry.yaml` 记录分层、别名、默认可发现性和运行时入口，但当前 `status: shadow`，Router 与生命周期不得按其裁剪发现面。
