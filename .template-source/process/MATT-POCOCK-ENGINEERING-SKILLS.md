...
- 当前战略设计 Harness 保留 `ask-matt`、`grill-with-docs`、`to-spec`、`to-tickets`、`triage` 和 `wayfinder` 作为用户显式兼容入口；任何 skill 不得自动调用它们，结果必须回交 `yss-strategic-design` 验收。`implement` 已移除，需转交下游研发 profile。
