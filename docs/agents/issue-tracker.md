...
5. 选定 GitHub / GitLab 但凭据或平台暂不可用时，先在 `docs/.scratch/<feature>/` 生成“待发布平台”草案；`map.md` 保留目标平台、标记 `publication: pending` 和 `pending_publication_to`；不得自动改投另一远程平台。
...
- `map.md` 导航至 Spec、设计、门禁、阻塞和阶段证据；front matter 的 `checkpoint_ref` 明确指向唯一 checkpoint，不复制机器状态。
- `issues/01-<slug>.md` 等文件是业务 Ticket 或 Wayfinder 子问题；不得把多个 Ticket 合并成一个文件。
- 每个 Ticket 在顶部附近使用 `Status:` 记录 Matt 五态之一：`needs-triage`、`needs-info`、`ready-for-human`、`wontfix`。
- 评论和对话追加在 `## Comments` 下；状态变化必须保留原因、证据引用和下一步。
- Wayfinder 的临时子问题可以沿用 Matt 的 `Status: claimed/resolved` 工作状态；它不是交付 Ticket 的五态角色，进入业务交付前必须转换为上述五态之一。
- Local 主 tracker 不要求远程 Ticket 存在；本地禁止 `ready-for-agent` 和技术切片。
- 显式调用 `to-tickets` 或原生业务 Ticket 正式化均生成业务任务，初始 `Status: ready-for-human`，回交战略主控。
...
所有平台都保留相同的业务 Ticket、依赖、验收、风险及确认语义；checkpoint 是唯一机器状态源，map.md 只引用和展示。平台差异只影响存储和操作方式，不改变生命周期阶段或门禁。
...
- Apply labels: `glab issue update <iid> --label "ready-for-human"`
...
- 创建或发布 Ticket：创建 `docs/.scratch/<feature>/`，按上述结构写入 `map.md`、`spec.md` 或 `issues/NN-<slug>.md`。
...
4. 选定远程平台不可用时，在 `docs/.scratch/<feature>/` 创建并标注“待发布平台”的草案；在 `map.md` 写入 `publication: pending` 和 `pending_publication_to`，恢复后优先补同步。
