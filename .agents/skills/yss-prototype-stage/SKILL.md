...
把产品设计影响收敛为跨 Agent 一致的原型交付物、证据和生命周期回流合同。它不替代当前 Harness 的生命周期编排器，也不把原型当作生产前端代码。

## 进入条件与主入口

- 先由当前 Harness 的生命周期编排器判断产品设计影响；无行为变化的孤立视觉修复记录 `not-applicable`，不创建空资产。
...
6. 以统一六轴 Design QA 和档位验证矩阵完成浏览器/无障碍验证。用户确认后只回交当前生命周期编排器，由当前 profile 决定继续形成 Strategic Design Handoff，或进入 API 影响分析与实现准备。
