...
export function assertGateChecks(gateId, state, { root = ROOT, registry = loadRegistry(), rolesDoc = loadDigitalHumanRoles() } = {}) {
  ...
  for (const id of gate.requires_gates || []) {
    const dependency = state.gates?.[id];
    if (!dependency) fail(`缺少前置门禁: ${id}`);
    if (dependency.status === 'not-applicable') {
      if (!dependency.reason?.trim() || !Array.isArray(dependency.evidence_refs) || !dependency.evidence_refs.length) fail(`${id} 未命中须有原因和证据`);
      continue;
    }
    if (dependency.status !== 'approved') fail(`前置门禁未批准: ${id}`);
    assertGateChecks(id, state, { root, registry, rolesDoc });
  }
