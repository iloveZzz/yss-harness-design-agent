...
- 建议门禁为 `gate.domain-strategy-approved` 和 `gate.stage-decision-package-approved`，实际状态由当前 profile 的主控维护：战略 profile 使用 `yss-strategic-design`，完整研发 profile 使用 `yss-product-lifecycle`；阶段包批准引用必须通过 `scripts/verify-approval-record`，不得用不可读路径或聊天确认替代。
