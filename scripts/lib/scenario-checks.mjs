...
function validateWorkflowExecutionResult(payload, contract, workUnitRoutes, options = {}) {
  ...
  if (payload.work_unit === "work-unit.slice-implementation" && payload.result === "completed") {
    ...
    const semantic = validateImplementationEntry(implementationState, {
      ...
      read: (ref) => ref === "docs/.scratch/demo/issues/01-valid-slice.md" ? "---\nkind: vertical-slice-ticket\n---\n# 合法切片测试\n" : ref === virtualTicketDecompositionRef ? virtualTicketDecomposition : readFileSync(path.join(root, ref), "utf8"),
    });
...
function validateInvocationProse(skill, adapter, orchestration) {
  ...
  ensure(adapter.includes("workflow reference 不表示调用") && adapter.includes("正式 Spec 或业务 Ticket 仍只能由对应显式用户入口创建"), "适配器未限制 workflow reference 的正式资产所有权");
